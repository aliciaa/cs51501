...
In order to avoid the invers of the projection matrix $P=I-BY(Y^TB^2Y)^{-1}Y^TB$, we need to find a orthonormal base $Q$ of $BY$ by QR-factorization. Let's note $Q=[Q_1, Q_2]$. It had been to prove that $P=I-Q_1Q_1^T$. Here I need to point out that the $Q1Q1^T$ should not be explicitly calculated out for otherwise we had to store and calculate a large dense matrix. Instead, Then we should store the tall and narrow matrix Q1, and the calculation of $P A$ should be replaced by $A-Q_1(Q_1^T\mathcal{A})$ calculated from the right to the left.  
