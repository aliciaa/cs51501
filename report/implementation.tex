...
We implemented the {\tt CG} linear solver for the linear system (eq.9) when A is s.p.d. And we implement some modifications to accelerate the prograss, since the linear solver would consume a large chunk of the time(as showed in Fig3).\\1. Avoid calculating the matrix inverse;\\2. Make it parallel for different linear equations;\\3. Do a convergence test to stop the iterations earlier.
