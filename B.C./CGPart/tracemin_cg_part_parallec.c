...
//#if(cxDEBUG==1)
  ...
  //ierr = MatView(E,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
//#endif
 /*The following is for the sequential method*/ 
  /*for(i=0;i<n;i++){
  ...
  /*The following is for the parallel method*/
  PetscScalar  lamda_max=1.0e-05;
  KSPCreate(PETSC_COMM_WORLD,&ksp);
  KSPSetOperators(ksp,A,SBP);
  KSPSetNormType(ksp,KSP_NORM_UNPRECONDITIONED);
  KSPSetCheckNormIteration(ksp,-1);
  KSPSetTolerances(ksp,lamda_max,PETSC_DEFAULT,PETSC_DEFAULT,10);
  KSPSetInitialGuessNonzero(ksp,PETSC_FALSE);
  KSPSetFromOptions(ksp);
  //VecCopy(sig,b);
  KSPSolve(ksp,b,x);
  //KSPSolve(ksp,(Vec)Y,x);
  KSPGetIterationNumber(ksp,&its);
  PetscPrintf(PETSC_COMM_WORLD,"final iteration number =%D\n",its);
  return 0;
